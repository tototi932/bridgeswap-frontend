...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% DGF', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your DGF cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% DGF', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
